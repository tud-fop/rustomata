...
pub enum TreeStackInstruction<A> {
    Up   { n: u8, current_val: A, old_val: A, new_val: A },
    Push { n: u8, current_val: A,             new_val: A },
    Down {        current_val: A, old_val: A, new_val: A }
}
...
impl<A: Ord + PartialEq + Clone + Debug>
    TreeStack<A> {
        ...
        pub fn new(a: A) -> TreeStack<A> {
            ...
            TreeStack { tree: tree, pointer: Vec::new() }
        }
        ...
        pub fn up(&self, n: u8, current_val: &A, old_val: Option<&A>, new_val: &A) -> Option<TreeStack<A>> {
            ...
            match (self.tree.get(&self.pointer), self.tree.get(&new_pointer).clone()) {
                (Some(val), o_val) if val == current_val && o_val == old_val =>
                {
                    ...
                    Some(TreeStack { tree: new_tree, pointer: new_pointer })
                }
                _ => None
            }
        }
