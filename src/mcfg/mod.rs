...
impl<N, T, W> Mcfg<N, T, W> {
    ...
    pub fn from_pmcfg(mut pmcfg: PMCFG<N, T, W>) -> Option<Self> {
        ...
        for rule in &pmcfg.rules {
            ...
            for (i, j) in rule.composition.composition.iter().flat_map(|component| component.iter().filter_map(|s| if let &VarT::Var(i,j) = s { Some((i, j)) } else { None })) {
        ...
        if pmcfg.initial.len() != 1 {
            ...
            Some(Mcfg{ rules: pmcfg.rules, initial: pmcfg.initial.pop().unwrap() })
        }
