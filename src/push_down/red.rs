...
impl<A: Ord + PartialEq + Debug + Clone + Hash,
     T: Eq + Clone + Hash,
     W: Ord + Eq + Clone + Add<Output=W> + Mul<Output = W> + Div<f64, Output = W> + Zero + One> Redundancy for PushDownAutomaton<A, T, W>{
    fn reduce_redundancy(self)-> PushDownAutomaton<A, T, W>{
        ...
        for (k,v) in tm{
            ...
            let mut number_of_redundancys = 0 as f64;
            for (_, tv) in c_map.clone(){
                let redundancies = tv.len().clone() as f64;
                if  redundancies > number_of_redundancys.clone(){
                    number_of_redundancys = redundancies.clone();
                }
            }
            //maps the new transitions. Weights are normalized by norm
            let norm = W::one().div(number_of_redundancys);
            for (tk, tv) in c_map{
                let mut nw = W::zero();
                for w in tv.clone(){
                    nw = nw + (w*norm.clone());
