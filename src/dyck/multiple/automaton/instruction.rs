...
impl<T: Clone + Ord> Instruction<TreeStack<MDTreeElem<T>>> for MultipleDyckInstruction<T> {
    fn apply(&self, ts: TreeStack<MDTreeElem<T>>) -> Vec<TreeStack<MDTreeElem<T>>> {
        use self::MultipleDyckInstruction::{Up, Down};
        ...
        match self {
            &Up(ref symbol, ref cell) => {
                ...
                succ.push(ts.clone().push_next(MDTreeElem::Node(Some(symbol.clone()), subset)));
            ...
            &Down(ref symbol) => {
                match ts.current_symbol() {
                    &MDTreeElem::Node(Some(ref symbol_), ref subset) => {
                        if symbol != symbol_ {
                            Vec::new()
                        } else if subset.is_empty() {  // pop
                            match ts.clone().pop() {
                                Ok(t) => vec![t],
                                _ => Vec::new()
                            }
                        } else {                // down
                            match ts.clone().set(MDTreeElem::Node(None, subset.clone())).down() {
                                Ok(t) => vec![t],
                                _ => Vec::new()
                            }
                        }
