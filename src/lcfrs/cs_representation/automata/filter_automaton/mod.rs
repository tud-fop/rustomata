...
impl<'a, T> Filter<T> where T: Hash + Eq + Clone + 'a {
          ...
          I: Integeriser<Item=PMCFGRule<N, T, W>>
    {
        ...
        Filter::Inside{ free_nts, free_brackets, nt, nt_and_t }
    }
    ...
    {
        match *self {
            Filter::Naive{ ref brackets_with, ref epsilon_brackets } => {
                let mut arcs = Vec::new();
                for i in 0..(word.len()) {
                    for &(ref terminals, i_brackets) in brackets_with.get(&word[i]).unwrap_or(&Vec::new()) {
                    ...
                    arcs.extend(epsilon_brackets.iter().map(
                        |brackets|
            ...
            Filter::Inside{ ref free_nts, ref free_brackets, ref nt, ref nt_and_t } => {
                ...
                for symbol in symbols {
                    for &(ref required_nts, ref ts_b_s, n) in nt_and_t.get(&symbol).unwrap_or(&Vec::new()) {
