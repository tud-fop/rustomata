...
use lcfrs::cs_representation::automata::FiniteAutomaton;
...
pub struct CykGenerator<T> where T: PartialEq {
...
where T: Clone + Hash + Eq
{
    pub fn new<W>(fsa: FiniteAutomaton<BracketFragment<T>, W>) -> Self {
        let FiniteAutomaton{ initial, arcs, finals, labels } = fsa;
        ...
        for (from, tos) in arcs.into_iter().enumerate() {
            for (ilabel, (to, weight)) in tos.into_iter() {
                ...
                if brackets.len() == 2 {
                    ...
                    match bracket {
                        Bracket::Open(cont) => { opening.push((from, cont, uniquestate)); },
                        Bracket::Close(cont) => { closing.entry(cont).or_insert_with(Vec::new).push((from, uniquestate)); }
                    }

                    let bracket = brackets.remove(0);
                    match bracket {
                        Bracket::Open(cont) => { opening.push((uniquestate, cont, to)); },
                        Bracket::Close(cont) => { closing.entry(cont).or_insert_with(Vec::new).push((uniquestate, to)); }
                    }
        ...
        for (from1, cont, to1) in opening {
            for &(from2, to2) in closing.get(&cont).unwrap_or(&Vec::new()) {
                bpairs.entry((to1, from2)).or_insert_with(Vec::new).push((from1, cont.clone(), to2));
            }
        }
...
where T: Clone + PartialEq
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        while self.unmarked.len() > 0 {
            ...
            // mark word
            self.marked_from_left.entry(from).or_insert_with(Vec::new).push((word.clone(), to));
            self.marked_from_right.entry(to).or_insert_with(Vec::new).push((from, word.clone()));
            ...
            for &(ref word_, to_) in self.marked_from_left.get(&to).unwrap_or(&Vec::new()) {
                self.unmarked.push((from, word.iter().chain(word_.iter()).cloned().collect(), to_));
            }
            for &(from_, ref word_) in self.marked_from_right.get(&from).unwrap_or(&Vec::new()) {
                self.unmarked.push((from_, word_.iter().chain(word.iter()).cloned().collect(), to));
            }

            // match bracket pairs around word
            for &(from_, ref bracket_content, to_) in self.bpairs.get(&(from, to)).unwrap_or(&Vec::new()) {
                self.unmarked.push((from_, once(Bracket::Open(bracket_content.clone())).chain(word.iter().cloned()).chain(once(Bracket::Close(bracket_content.clone()))).collect(), to_));
            }
...
mod tests {
    ...
    use lcfrs::cs_representation::automata::finite_automaton::StateInstruction;

    #[test]
    fn test_cykgen_construction() {
        let should_be = CykGenerator{ initial: 0,
                                      finals: vec![1],
                                      
                                      unmarked: vec![(0, vec![Bracket::Open(BracketContent::Component(1, 0)), Bracket::Open(BracketContent::Terminal("a".to_owned())), Bracket::Close(BracketContent::Terminal("a".to_owned())), Bracket::Close(BracketContent::Component(1, 0)) ], 1)],
                                      marked_from_left: HashMap::new(),
                                      marked_from_right: HashMap::new(),
    ...
    fn test_cykgen_iterator() {
        assert_eq!(
            ...
            vec![
                vec![
                    Bracket::Open(BracketContent::Component(1, 0)), Bracket::Open(BracketContent::Terminal("a".to_owned())),
                    Bracket::Close(BracketContent::Terminal("a".to_owned())), Bracket::Close(BracketContent::Component(1, 0)),
                ],
                vec![
                    Bracket::Open(BracketContent::Component(0, 0)), Bracket::Open(BracketContent::Variable(0, 0, 0)),
                    Bracket::Open(BracketContent::Component(1, 0)), Bracket::Open(BracketContent::Terminal("a".to_owned())),
                    Bracket::Close(BracketContent::Terminal("a".to_owned())), Bracket::Close(BracketContent::Component(1, 0)),
                    Bracket::Close(BracketContent::Variable(0, 0, 0)), Bracket::Close(BracketContent::Component(0, 0))
                ]
            ]
        )
    }
    ...
    fn example_fsa() -> FiniteAutomaton<BracketFragment<String>, LogDomain<f64>> {
        let arcs = vec![
            Transition{ weight: LogDomain::new(1.0).unwrap(),
                        instruction: StateInstruction(0, 0),
                        word: vec![ BracketFragment(vec![Bracket::Open(BracketContent::Component(0, 0)), Bracket::Open(BracketContent::Variable(0, 0, 0))]) ]
                      },
            Transition{ weight: LogDomain::new(1.0).unwrap(),
                        instruction: StateInstruction(0, 1),
                        word: vec![ BracketFragment(vec![Bracket::Open(BracketContent::Component(1, 0)), Bracket::Open(BracketContent::Terminal("a".to_owned())), Bracket::Close(BracketContent::Terminal("a".to_owned())), Bracket::Close(BracketContent::Component(1, 0)) ])]
                      },
            Transition{ weight: LogDomain::new(1.0).unwrap(),
                        instruction: StateInstruction(1, 1),
                        word: vec![ BracketFragment(vec![Bracket::Close(BracketContent::Variable(0, 0, 0)), Bracket::Close(BracketContent::Component(0, 0))]) ]
                      },
        ];
