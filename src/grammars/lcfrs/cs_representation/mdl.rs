    ...
    N: 'a,
{
    ...
    for rule in lcfrs {
        ...
        partition.extend((0..rule.tail.len()).map(|successor| {
            rule.composition
                .composition
                .iter()
                .flat_map(|component| {
                    component.iter().filter_map(
                        |vart| if let &VarT::Var(i, j) = vart {
                            if i == successor {
                                Some(BracketContent::Variable(rule_id, i, j))
                            } else {
                                None
                            }
                        } else {
    ...
    MultipleDyckLanguage::sorted(Partition::new(partition).unwrap(), |symbol| {
